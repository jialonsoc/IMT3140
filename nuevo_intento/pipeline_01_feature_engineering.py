...
def _extract_one_synthetic(row: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    uc_path = SYNTH_DIR / f"{synthetic_record}_uc.npy"
...
def build_synthetic_windows(max_workers: int | None = None) -> pd.DataFrame:
    ...
    Synthetic FHR is paired with its synchronized bootstrap UC. The UC files are
    generated by nuevo_intento/pipeline_00_generate_synthetic_uc.py.
    """
