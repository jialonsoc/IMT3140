...
def select_winning_xgboost(report: pd.DataFrame) -> str:
    """Select the XGBoost variant with the highest test AUPRC, then AUROC."""
    candidates = report[report["Model"].astype(str).str.startswith("XGBoost -")].copy()
    if candidates.empty:
        raise ValueError("El reporte final no contiene modelos XGBoost.")
    candidates = candidates.sort_values(["AUPRC", "AUROC"], ascending=False)
    return str(candidates.iloc[0]["Model"])


def plot_metric_comparison(report: pd.DataFrame, winning_model: str) -> Path:
...
def _load_winning_model_config(winning_model: str) -> tuple[list[str], dict[str, Any], float]:
...
def fit_winning_xgb_predictions(winning_model: str) -> tuple[pd.DataFrame, float]:
    """Fit the winning XGBoost and return probabilities for real test windows."""
    features, best_params, threshold = _load_winning_model_config(winning_model)
    ...
    experiment_name = (
        "Sequential real+synthetic"
        if winning_model.endswith("real+synthetic")
        else "Sequential real-only"
    )
    winning_train = next(exp.train_df for exp in experiments if exp.name == experiment_name)
    y_train = winning_train["target"].astype(int)
    ...
    estimator.fit(winning_train[selected_features], y_train)
...
def plot_winning_feature_importance(winning_model: str, top_n: int = 20) -> Path:
    ...
    ax.text(
        0.99,
        0.03,
        f"Modelo: {MODEL_LABELS.get(winning_model, winning_model)}",
        transform=ax.transAxes,
        ha="right",
        va="bottom",
        color="#374151",
...
def main() -> None:
    ...
    winning_model = select_winning_xgboost(report)
    test_predictions, threshold = fit_winning_xgb_predictions(winning_model)
